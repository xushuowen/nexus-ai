...
class AutoScheduleSkill(BaseSkill):
    ...
    intent_patterns = [
        r"每(天|日|週|月).{0,10}(幫我|自動|提醒|執行|發送|通知)",
        r"固定.{0,5}(每天|每週|每日).{0,15}",
        r"(早上|下午|晚上|凌晨)\d{1,2}點.{0,15}(自動|定時|幫我)",
        r"設定.{0,5}(每天|每週|循環|定時|固定)",
        r"(每週[一二三四五六日]|週[一二三四五六日]).{0,10}(提醒|幫我|執行)",
    ]
    category = "automation"
    requires_llm = False
